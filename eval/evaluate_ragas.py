...
# Setup local Ollama for judge evaluation (Tier 2) - using a smaller model
# for speed
llm = ChatOllama(
    model="llama3.2:3b",
    temperature=0,
...
def compute_semantic_similarity(df):
    ...
    for a, r in zip(ans_emb, ref_emb):
        sim = cosine_similarity(np.array(a).reshape(
            1, -1), np.array(r).reshape(1, -1))[0][0]
...
def main():
    if not os.path.exists(
            "eval/baseline_results.csv") or not os.path.exists("eval/hybrid_results.csv"):

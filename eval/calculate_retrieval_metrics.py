...
def calculate_mrr(retrieved_texts, ground_truth_text):
    for i, text in enumerate(retrieved_texts):
        # We use a simple substring match or high overlap since exact match might fail if whitespace differs slightly
        if ground_truth_text.strip() in text.strip() or text.strip() in ground_truth_text.strip():
            return 1.0 / (i + 1)
    return 0.0
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        # 1. Baseline Search
        baseline_results = search_chroma(query=question, collection=coll, n_results=5)
        baseline_texts = [r["text"] for r in baseline_results]

        baseline_mrr.append(calculate_mrr(baseline_texts, ground_truth_context))
        baseline_recall_1.append(calculate_recall_at_k(baseline_texts, ground_truth_context, 1))
        baseline_recall_3.append(calculate_recall_at_k(baseline_texts, ground_truth_context, 3))
        baseline_recall_5.append(calculate_recall_at_k(baseline_texts, ground_truth_context, 5))

        # 2. Hybrid Search
        hybrid_results = search_chroma_hybrid(query=question, collection=coll, n_results=5)
        hybrid_texts = [r["text"] for r in hybrid_results]

        hybrid_mrr.append(calculate_mrr(hybrid_texts, ground_truth_context))
        hybrid_recall_1.append(calculate_recall_at_k(hybrid_texts, ground_truth_context, 1))
